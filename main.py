...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ret == -1:
            ...
            print 'capture ok'
            #retcode,js = cloud_ocr.do_ocr(f1,config_json['bd_ocr_ak'],config_json['bd_ocr_sk'])
            #if retcode == 0:
            #    for d in js:
            #        #print d['words']
            #        if len(d['words']) == 3 and d['words'].isnumeric() == True:
            #            print 'data:',int(d['words'])
            #            utils.log_dump('label.txt',f1 + ',' + str(int(d['words'])))
            #            break
            #else:
            #    print 'do_ocr error',retcode,js
